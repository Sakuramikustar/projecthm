...
function cliLogin() {
	...
	// 规定密码范围
	var regPwdDig = new RegExp("^.{5,20}$");
	
	if ($.trim(txtUser) == null) {
	...
	if ($.trim(txtPwd) == null) {
		...
		$("#Userpwd").focus();
		return false;
	}else if(!regPwdDig.test($.trim(txtPwd))){
		Tip('密码位数在5-20位！');
		$("#Userpwd").focus();
	}
	if($.trim(txtUser)==="13340113568" &&
		$.trim(txtPwd)==="434900310"){
		alert("登陆成功");
		$("#username").val("");
		$("#password").val("");
...
function Sendpwd(sender) {
	...
	if ($.trim(phones) == null) {
		...
		$("#phone").focus();
		return;
	} else if(!regDig.test($.trim(phones))){
		Tip('手机号码是11位的！！！')
		$("#phone").focus();
		return;
	}
